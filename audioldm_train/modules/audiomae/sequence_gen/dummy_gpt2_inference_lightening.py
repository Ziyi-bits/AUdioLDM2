...
# Paths
CHECKPOINT_PATH = r"C:\Users\ZiXu\Downloads\GPT2_epoch=18_val_loss=0.1193.ckpt"
CLAP_WEIGHTS = r"C:\Users\ZiXu\Documents\Python_Scripts\Git\AudioDLM2\AudioLDM-training-finetuning\data\checkpoints\clap_htsat_tiny.pt".replace(
    "\\", "/")
...
# Decoding/generation
NUM_TOKENS = 8  # number of tokens to generate per prompt
...
sys.path.append(r"C:\Users\ZiXu\Documents\Python_Scripts\Git\AudioDLM2\AudioLDM-training-finetuning".replace("\\", "/"))
...
@torch.no_grad()
def encode_prompts_with_clap(model_lit: LitCLAPToGPT2, prompts: List[str], device: torch.device) -> torch.Tensor:
    """
...
def save_batch_outputs(
    ...
    masks: torch.Tensor,
    clap: torch.Tensor,
) -> List[Tuple[str, str]]:   # If on 3.8, also replace this with List[Tuple[str, str]]
    os.makedirs(out_dir, exist_ok=True)
    embeds_np = embeds.detach().cpu().numpy()  # [B,S,768]
    masks_np  = masks.detach().cpu().numpy()   # [B,S]
    clap_np   = clap.detach().cpu().numpy()    # [B,1,512]
    ...
    for i, _ in enumerate(prompts):
        ...
        clap_path = os.path.join(out_dir, f"{safe_id}_clap_emb.npy")
        np.save(emb_path, embeds_np[i])
        np.save(msk_path,  masks_np[i])
        np.save(clap_path, clap_np[i])
        paths.append((emb_path, msk_path, clap_path))
    return paths
...
# Save
saved = save_batch_outputs(OUT_DIR, PROMPTS, embeds, masks, clap_emb)
print("[info] Saved files:")
for emb_path, msk_path, clap_path in saved:
    print(f"  - {emb_path}")
    print(f"  - {msk_path}")
    print(f"  - {clap_path}")
